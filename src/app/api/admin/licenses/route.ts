...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) query = query.or(`name.ilike.%${search}%,city.ilike.%${search}%,phone.ilike.%${search}%,license_key.ilike.%${search}%,email.ilike.%${search}%`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, city, phone, type, features, status, rent_amount, notes, email, telephone, logo } = body;
    ...
    const { data, error } = await getSupabaseAdmin().from('licenses').insert({
      name, city: city || null, phone: phone || null, type: type || 'hospital',
      license_key: licenseKey, username, password, features: features || [],
      status: status || 'active', rent_amount: rent_amount || 0, notes: notes || null,
      email: email || null, telephone: telephone || null, logo: logo || null,
    }).select().single();
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
